#! /usr/bin/gawk -f
#
# Copyright (c) 2009 - 2025 Christian W. Moenneckes
#
# vim:sts=4:ts=8:sw=4:expandtabs
    ...
    if (ENVIRON["SERVER_PROTOCOL"] ~ /^HTTP/) {
        ...
        if (ENVIRON["SELF"]) {
            ...
            SELF = ((ENVIRON["HTTPS"] ~ /^on$/) ? "https://" : "http://") \
                ENVIRON["HTTP_HOST"] ENVIRON["SCRIPT_NAME"];
        }

        if (ENVIRON["REQUEST_METHOD"] == "POST") {
            query= "";
            if (ENVIRON["CONTENT_TYPE"] ~ /^multipart\/form-data/) {
                getline boundary < ("/dev/stdin");
                gsub(/\r/, "", boundary);
                while (line != boundary "--\r") {
                    var = "";
                    while (getline line < ("/dev/stdin") && line != "\r") {
                        gsub(/\r/, "", line);
#                        printf "GET a head: %s\n", line >> "/dev/stderr";
...
#                        printf "GET a data for '%s'\n", var >> "/dev/stderr";
        ...
        if (! ENVIRON["PATH_INFO"]) {
            exit_redirect(OPT["DEFAULT_DOC"], 1);
        } else if (CGI["doc"]) {
            while (match(CGI["doc"], /^(.*)[ _]+([A-Z].*)/,ary))
                CGI["doc"] = ary[1] ary[2];

            exit_redirect(CGI["doc"], 1);
...
#    print "<!DOCTYPE html PUBLIC '-//W3C//DTD HTML 4.01 Transitional//EN' 'http://www.w3.org/TR/html4/loose.dtd'>";
    ...
    html_tag("link", sprintf("rel='stylesheet' media='screen' type='text/css' href='%s/static/wiki.css'", OPT["SITE_URL"]));

    if (OPT["ICON"])
        html_tag("link", "rel='icon' type='image/png' href='" OPT["ICON"] "'");

    html_tag("script", sprintf("src='%s/static/wiki.js' type='text/javascript'", OPT["SITE_URL"]), " ");
...
#    html_close("head");
    ...
    if (CGI["debug"] >= 2) {
        ...
        printf "PROCINFO\n"
        for (v in PROCINFO)
            if (v != "argv" && v != "identifiers")
                print raw2html(" " v " " PROCINFO[v], 1);
    ...
    if (ENVIRON["SERVER_PROTOCOL"] ~ /^HTTP/ && TAG[0]) {
        ...
        if (RW && (CGI["mode"] == "edit" \
            ...
            for (meta in KEYVALUE) {
                if (! KEYVALUE[meta]) continue;
                #if (! check_meta(meta, KEYVALUE[meta], DOC, 0)) {
                if (! check_meta(meta, KEYVALUE[meta], DOCNAME, 0)) {
...
function exit_redirect(doc, exit_code) {
